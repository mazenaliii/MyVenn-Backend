...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    if (existingUser) {
      return res.status(400).json({ errMessage: "The user with this email already registered before.", success });
    }
    ...
    await newUser.save();
    success = true;
    res.status(201).json({ successMessage: "Account created. Welcome to MyVenn!", success });
  } catch (error) {
